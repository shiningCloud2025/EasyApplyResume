"""
全新：白色主题 + 修复动画 (cond 里加 sldTgt + 用 p:set 实现淡入)
"""
...
# ============ 白色统一配色 ============
BG       = RGBColor(0xF8, 0xFA, 0xFC)   # 极浅灰白底
CARD     = RGBColor(0xFF, 0xFF, 0xFF)   # 纯白卡片
BLUE     = RGBColor(0x02, 0x84, 0xC7)   # 深蓝
TEAL     = RGBColor(0x0D, 0x94, 0x8F)   # 墨绿青
ORANGE   = RGBColor(0xEA, 0x58, 0x0C)   # 深橙
PURPLE   = RGBColor(0x7C, 0x3A, 0xED)   # 紫色
RED      = RGBColor(0xDC, 0x26, 0x26)   # 红色
DARK     = RGBColor(0x0F, 0x17, 0x2A)   # 深色（标题栏用）
TEXT     = RGBColor(0x1E, 0x29, 0x3B)   # 正文深色
GRAY     = RGBColor(0x64, 0x74, 0x8B)   # 灰色
LIGHT    = RGBColor(0xE2, 0xE8, 0xF0)   # 浅灰分隔线
...
def add_text(slide, l, t, w, h, text, size=12, color=TEXT, bold=False,
...
def add_title_bar(slide, icon_text):
    add_rect(slide, Inches(0), Inches(0), SLIDE_W, Inches(0.8), DARK)
    add_text(slide, Inches(0.6), Inches(0.15), Inches(12), Inches(0.5),
             icon_text, size=22, color=RGBColor(0xFF,0xFF,0xFF), bold=True,
             font='Trebuchet MS', va=MSO_ANCHOR.MIDDLE)
    add_rect(slide, Inches(0), Inches(0.8), SLIDE_W, Inches(0.03), BLUE)

def add_page_num(slide, n):
    add_text(slide, Inches(11.5), Inches(7.1), Inches(1.5), Inches(0.3),
             f"{n}/9", size=9, color=GRAY, align=PP_ALIGN.RIGHT)

# ============ 动画注入（修复版） ============
def make_onclick_set(shape_id):
    """
    创建点击触发的 "出现" 动画 (p:set)
    关键修复: cond 里加 sldTgt 作为触发目标
    """
    ...
    cond.set("delay", "0")
    # 关键：指定触发目标为幻灯片自身（点击任意位置触发）
    tgtEl_cond = etree.SubElement(cond, f"{{{P_NS}}}tgtEl")
    sldTgt = etree.SubElement(tgtEl_cond, f"{{{P_NS}}}sldTgt")
    
    childTnLst = etree.SubElement(par, f"{{{P_NS}}}childTnLst")
    
    # 用 p:set 实现"出现"效果（最可靠的入场动画）
    set_elem = etree.SubElement(childTnLst, f"{{{P_NS}}}set")
    ...
    # 设置可见性
    attrNameLst = etree.SubElement(set_elem, f"{{{P_NS}}}attrNameLst")
    attrName = etree.SubElement(attrNameLst, f"{{{P_NS}}}attrName")
    attrName.text = "style.visibility"
    
    to = etree.SubElement(set_elem, f"{{{P_NS}}}to")
    toCh = etree.SubElement(to, f"{{{P_NS}}}strVal")
    toCh.set("val", "visible")
    
    return par

def add_animations(slide, shape_ids, trans='fade'):
    """
    shape_ids: [(shape_id, 'fade'), ...]
    """
    # 翻页过渡
    ...
    if trans in tmap:
        p = etree.SubElement(trans_el, f"{{{P_NS}}}{trans}")
        p.set("dir", tmap[trans])
    elif trans == 'dissolve':
    ...
    for sid, _ in shape_ids:
        par = make_onclick_set(sid)
...
tb1 = add_text(s1, Inches(1.5), Inches(2.0), Inches(10), Inches(1.0),
...
tb3 = add_text(s1, Inches(1.5), Inches(4.5), Inches(10), Inches(0.5),
         "Spring Boot · Spring AI · MyBatis-Plus · Redis · PostgreSQL · React · Docker",
         size=13, color=GRAY)

ids1 = []
for s in s1.shapes:
    ids1.append(s.shape_id)
add_animations(s1, [(ids1[0],'fade'),(ids1[1],'fade'),(ids1[2],'fade'),(ids1[3],'fade'),(ids1[4],'fade')], 'fade')
...
for i, (title, desc, accent) in enumerate(problems):
    ...
    c = add_rrect(s2, x, y, Inches(5.8), Inches(2.4), CARD)
    ids2.append(c.shape_id)
    add_rect(s2, x, y, Inches(0.06), Inches(2.4), accent)
    add_text(s2, x + Inches(0.3), y + Inches(0.2), Inches(5.2), Inches(0.45),
             title, size=18, color=accent, bold=True, font='Trebuchet MS')
    add_text(s2, x + Inches(0.3), y + Inches(0.85), Inches(5.2), Inches(1.3),
             desc, size=14, color=TEXT)

ids2_full = [(s.shape_id, 'fade') for s in s2.shapes]
add_animations(s2, ids2_full, 'push')
...
for ci, (cat, items, accent) in enumerate(categories):
    ...
    add_text(s3, x + Inches(0.1), Inches(1.2), Inches(3.6), Inches(0.5),
             cat, size=15, color=RGBColor(0xFF,0xFF,0xFF), bold=True,
             font='Trebuchet MS', align=PP_ALIGN.CENTER, va=MSO_ANCHOR.MIDDLE)
    for j, item in enumerate(items):
        iy = Inches(2.0 + j * 0.55)
        add_rrect(s3, x, iy, Inches(3.8), Inches(0.45), CARD)
        add_text(s3, x + Inches(0.15), iy + Inches(0.08), Inches(3.5), Inches(0.3),
                 item, size=11, color=TEXT, va=MSO_ANCHOR.MIDDLE)

ids3 = [(s.shape_id, 'fade') for s in s3.shapes]
add_animations(s3, ids3, 'cover')
...
for i, (name, desc, accent) in enumerate(agents):
    ...
    add_text(s4, Inches(1.0), y + Inches(0.15), Inches(5), Inches(0.4),
             name, size=20, color=accent, bold=True, font='Trebuchet MS')
    add_text(s4, Inches(1.0), y + Inches(0.6), Inches(5), Inches(0.5),
             desc, size=12, color=GRAY)
    if i < 2:
        add_text(s4, Inches(3), y + Inches(1.15), Inches(1), Inches(0.3),
                 "⬇", size=16, color=GRAY, align=PP_ALIGN.CENTER)

add_text(s4, Inches(6.8), Inches(1.2), Inches(6), Inches(0.5),
...
for i, (label, desc, accent) in enumerate(comps):
    ...
    add_text(s4, Inches(7.1), y + Inches(0.38), Inches(5.4), Inches(0.3),
             desc, size=11, color=GRAY)

ids4 = [(s.shape_id, 'fade') for s in s4.shapes]
add_animations(s4, ids4, 'wipe')
...
for i, (title, desc, accent) in enumerate(feats):
    ...
    add_text(s5, x + Inches(0.2), y + Inches(0.25), Inches(3.4), Inches(0.45),
             title, size=16, color=accent, bold=True, font='Trebuchet MS')
    add_text(s5, x + Inches(0.2), y + Inches(0.9), Inches(3.4), Inches(1.3),
             desc, size=12, color=TEXT)

ids5 = [(s.shape_id, 'fade') for s in s5.shapes]
add_animations(s5, ids5, 'uncover')
...
for i, (title, desc) in enumerate(mods):
    ...
    add_text(s6, Inches(1.0), y + Inches(0.1), Inches(5.8), Inches(0.3),
             title, size=16, color=BLUE, bold=True, font='Trebuchet MS')
    add_text(s6, Inches(1.0), y + Inches(0.45), Inches(5.8), Inches(0.4),
             desc, size=11, color=GRAY)

add_text(s6, Inches(7.6), Inches(1.15), Inches(5.5), Inches(0.45),
...
for i, item in enumerate(["智能数据统计与分析", "自动化运营建议", "异常行为检测预警",
    ...
    add_text(s6, Inches(7.6), Inches(1.8 + i * 0.5), Inches(5.5), Inches(0.4),
             f"✦ {item}", size=13, color=TEXT)

ids6 = [(s.shape_id, 'fade') for s in s6.shapes]
add_animations(s6, ids6, 'fade')
...
for i, (title, desc, accent) in enumerate(dfs):
    ...
    add_text(s7, x + Inches(0.3), y + Inches(0.25), Inches(5.2), Inches(0.5),
             title, size=20, color=accent, bold=True, font='Trebuchet MS')
    add_text(s7, x + Inches(0.3), y + Inches(1.0), Inches(5.2), Inches(1.3),
             desc, size=14, color=TEXT)

ids7 = [(s.shape_id, 'fade') for s in s7.shapes]
add_animations(s7, ids7, 'push')
...
for title, desc, accent, x in phases:
    ...
    add_text(s8, x + Inches(0.3), Inches(1.6), Inches(3.2), Inches(0.5),
             title, size=22, color=accent, bold=True, font='Trebuchet MS',
             align=PP_ALIGN.CENTER)
    add_text(s8, x + Inches(0.3), Inches(2.4), Inches(3.2), Inches(3),
             desc, size=16, color=TEXT, align=PP_ALIGN.CENTER)

bott = add_rect(s8, Inches(1), Inches(6.4), Inches(11.333), Inches(0.5), BLUE)
add_text(s8, Inches(1), Inches(6.4), Inches(11.333), Inches(0.5),
         "让每个人都能找到理想的工作 — AI 赋能求职全流程",
         size=16, color=RGBColor(0xFF,0xFF,0xFF), bold=True, font='Trebuchet MS',
         align=PP_ALIGN.CENTER, va=MSO_ANCHOR.MIDDLE)

ids8 = [(s.shape_id, 'fade') for s in s8.shapes]
add_animations(s8, ids8, 'cover')
...
add_text(s9, Inches(1), Inches(2.3), Inches(11.333), Inches(1),
         "感谢聆听", size=52, color=DARK, bold=True, font='Trebuchet MS',
         align=PP_ALIGN.CENTER)
add_text(s9, Inches(1), Inches(3.6), Inches(11.333), Inches(0.6),
         "EasyApplyResume — AI 驱动的智能求职与管理平台",
         size=18, color=BLUE, font='Trebuchet MS', align=PP_ALIGN.CENTER)
add_text(s9, Inches(1), Inches(5.0), Inches(11.333), Inches(0.4),
         "Spring Boot · Spring AI · MyBatis-Plus · Redis · PostgreSQL · React · Docker",
         size=12, color=GRAY, align=PP_ALIGN.CENTER)
add_text(s9, Inches(1), Inches(5.6), Inches(11.333), Inches(0.4),
         "shiningCloud2025 © 2026", size=11, color=GRAY, align=PP_ALIGN.CENTER)

ids9 = [(s.shape_id, 'fade') for s in s9.shapes]
add_animations(s9, ids9, 'dissolve')


# ============ 保存 & 验证 ============
out = "out/EasyApplyResume-项目介绍-v7.pptx"
prs.save(out)
        ...
        for p in pars:
            for cond in p.findall(f'.//{{{P_NS}}}cond'):
                if cond.get('evt') == 'onclick':
                    has_click = True
                if cond.find(f'{{{P_NS}}}tgtEl/{{{P_NS}}}sldTgt') is not None:
                    has_sldtgt = True
        total += len(pars)
        print(f"  Slide {i+1}: trans=YES, anims={len(pars)}, onclick={has_click}, sldTgt={has_sldtgt}")
    else:
        print(f"  Slide {i+1}: NO timing!")

print(f"\nDone: {len(prs2.slides)} pages, {total} onclick + sldTgt animations -> {out}")
