...
public class AdminFormValidator {
    ...
    // 邮箱默认值（未填写时）
    private static final String DEFAULT_EMAIL = "待完善";

    /**
     * 新增管理员校验规则：
     * - 强制必填：账号名、手机号
     * - 选填+默认：邮箱（未填→"待完善"，填了→校验格式/长度）
    ...
    public static void validateForAdd(AdminForm adminForm) {
        ...
        // 3. 选填+默认：邮箱（未填→"待完善"，填了→校验）→ ADMIN_EMAIL_TOO_LONG(606) / ADMIN_EMAIL_FORMAT_ERROR(604)
        String email = adminForm.getAdminEmail();
        if (email == null || email.trim().isEmpty()) {
            adminForm.setAdminEmail(DEFAULT_EMAIL); // 默认值
        } else {
            email = email.trim();
        ...
        // 7. 辅助字段：状态（默认1-正常+校验合法性）→ ADMIN_STATE_ILLEGAL(618)
    ...
    public static void validateForUpdate(AdminForm adminForm) {
        // 1. 账号名：必须非空 + 长度≤15位 → ADMIN_UPDATE_USERNAME_EMPTY(611) / ADMIN_USERNAME_TOO_LONG(605)
        if (adminForm.getAdminUsername() == null || adminForm.getAdminUsername().trim().isEmpty()) {
        ...
        // 2. 手机号：必须非空 + 11位有效数字 → ADMIN_UPDATE_PHONE_EMPTY(612) / ADMIN_PHONE_FORMAT_ERROR(603)
        if (adminForm.getAdminPhone() == null || adminForm.getAdminPhone().trim().isEmpty()) {
        ...
        // 3. 邮箱：必须非空 + 格式正确 + 长度≤25位 → ADMIN_UPDATE_EMAIL_EMPTY(613) / ADMIN_EMAIL_TOO_LONG(606) / ADMIN_EMAIL_FORMAT_ERROR(604)
        ...
        // 4. 密码：必须非空 + 长度6-20位 → ADMIN_UPDATE_PASSWORD_EMPTY(614) / ADMIN_PASSWORD_LENGTH_ERROR(608)
        if (adminForm.getAdminPassword() == null || adminForm.getAdminPassword().trim().isEmpty()) {
        ...
        // 5. 头像：必须非空（不能是空串）→ ADMIN_UPDATE_IMAGE_EMPTY(615)
        if (adminForm.getAdminImage() == null || adminForm.getAdminImage().trim().isEmpty()) {
            throw new BusException(CodeEnum.ADMIN_UPDATE_IMAGE_EMPTY);
        }

        // 6. 介绍：必须非空 + 长度≤200位 → ADMIN_UPDATE_INTRO_EMPTY(616) / ADMIN_INTRO_TOO_LONG(607)
        ...
        // 7. 状态：必须非空 + 只能是0/1 → ADMIN_UPDATE_STATE_EMPTY(617) / ADMIN_STATE_ILLEGAL(618)
        if (adminForm.getAdminState() == null) {
