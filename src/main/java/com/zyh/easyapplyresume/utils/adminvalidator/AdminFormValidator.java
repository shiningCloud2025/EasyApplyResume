...
public class AdminFormValidator {
    ...
    public static void validateForAdd(AdminForm adminForm) {
        ...
        if (account.length() < ACCOUNT_MIN_LENGTH || account.length() > ACCOUNT_MAX_LENGTH) {
            throw new BusException(AdminCodeEnum.ADMIN_ACCOUNT_LENGTH_ERROR);
        }
        adminForm.setAdminAccount(account);

        // 2. 强制必填：账号名（非空+长度1-15位）→ ADMIN_ADD_USERNAME_EMPTY(609)
        ...
        // 3. 强制必填：手机号（非空+格式正确）→ ADMIN_ADD_PHONE_EMPTY(610) / ADMIN_PHONE_FORMAT_ERROR(603)
        ...
        // 4. 强制必填：邮箱（非空+格式正确+长度≤25位）→ ADMIN_ADD_EMAIL_EMPTY(611) / ADMIN_EMAIL_TOO_LONG(606) / ADMIN_EMAIL_FORMAT_ERROR(604)
        ...
        // 5. 辅助字段：密码（默认123456，填了则校验长度）→ ADMIN_PASSWORD_LENGTH_ERROR(608)
        ...
        // 7. 辅助字段：介绍（默认文案+自定义≤200字）→ ADMIN_INTRO_TOO_LONG(607)
        ...
        // 8. 辅助字段：状态（默认1-正常+校验合法性）→ ADMIN_STATE_ILLEGAL(619)
    ...
    public static void validateForUpdate(AdminForm adminForm) {
        ...
        // 2. 账号名：必须非空 + 长度≤15位 → ADMIN_UPDATE_USERNAME_EMPTY(612) / ADMIN_USERNAME_TOO_LONG(605)
        if (adminForm.getAdminUsername() == null || adminForm.getAdminUsername().trim().isEmpty()) {
        ...
        // 3. 手机号：必须非空 + 11位有效数字 → ADMIN_UPDATE_PHONE_EMPTY(613) / ADMIN_PHONE_FORMAT_ERROR(603)
        if (adminForm.getAdminPhone() == null || adminForm.getAdminPhone().trim().isEmpty()) {
        ...
        // 4. 邮箱：必须非空 + 格式正确 + 长度≤25位 → ADMIN_UPDATE_EMAIL_EMPTY(614) / ADMIN_EMAIL_TOO_LONG(606) / ADMIN_EMAIL_FORMAT_ERROR(604)
        if (adminForm.getAdminEmail() == null || adminForm.getAdminEmail().trim().isEmpty()) {
        ...
        // 5. 密码：必须非空 + 长度6-20位 → ADMIN_UPDATE_PASSWORD_EMPTY(615) / ADMIN_PASSWORD_LENGTH_ERROR(608)
        if (adminForm.getAdminPassword() == null || adminForm.getAdminPassword().trim().isEmpty()) {
        ...
        // 6. 头像：必须非空（不能是空串）→ ADMIN_UPDATE_IMAGE_EMPTY(616)
        if (adminForm.getAdminImage() == null || adminForm.getAdminImage().trim().isEmpty()) {
            throw new BusException(AdminCodeEnum.ADMIN_UPDATE_IMAGE_EMPTY);
        }

        // 7. 介绍：必须非空 + 长度≤200位 → ADMIN_UPDATE_INTRO_EMPTY(617) / ADMIN_INTRO_TOO_LONG(607)
        ...
        // 8. 状态：必须非空 + 只能是0/1 → ADMIN_UPDATE_STATE_EMPTY(618) / ADMIN_STATE_ILLEGAL(619)
        if (adminForm.getAdminState() == null) {
