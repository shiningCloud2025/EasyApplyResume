...
/**
 * 管理端广告表单检查工具类，用于验证广告表单数据
 * 支持新增场景和修改场景的字段校验
 * @author shiningCloud2025
 */
public class AdmonitorAdminAdvertisementValidator {
     ...
     * @param admonitorAdminAdvertisementForm 待验证的广告表单对象
     * @throws BusException 字段不满足规则时抛出
     */
    public static void validateForAdd(AdmonitorAdminAdvertisementForm admonitorAdminAdvertisementForm) {
        // 通用字段校验（新增/修改一致）
        validateCommonFields(admonitorAdminAdvertisementForm);
    }
     ...
     * @throws BusException 字段不满足规则时抛出
     */
    public static void validateForUpdate(AdmonitorAdminAdvertisementForm admonitorAdminAdvertisementForm) {
        // 通用字段校验（新增/修改一致）
        validateCommonFields(admonitorAdminAdvertisementForm);
        // 额外：修改场景可添加主键校验（如advertisementId非空），根据业务需求扩展
        validateAdvertisementId(admonitorAdminAdvertisementForm);
    }
     ...
     * @throws BusException 字段不满足规则时抛出
     */
    private static void validateCommonFields(AdmonitorAdminAdvertisementForm form) {
        // 1. 校验广告名称
     ...
     */
    private static void validateAdvertisementName(AdmonitorAdminAdvertisementForm form) {
     ...
     */
    private static void validateAdvertisementUrl(AdmonitorAdminAdvertisementForm form) {
     ...
     */
    private static void validateAdvertisementLink(AdmonitorAdminAdvertisementForm form) {
     ...
     */
    private static void validateAdvertisementTime(AdmonitorAdminAdvertisementForm form) {
     ...
     * 可根据业务需求扩展，比如主键非空、主键存在于数据库等
     * @param form 待验证的广告表单对象
     * @throws BusException 主键不满足规则时抛出
     */
    private static void validateAdvertisementId(AdmonitorAdminAdvertisementForm form) {
        Integer id = form.getAdvertisementId();
        if (id == null || id <= 0) {
            // 可新增对应的枚举项，比如ADVERTISEMENT_ID_ILLEGAL(20028, "广告ID不合法")
            throw new BusException(AdMonitorCodeEnum.ADVERTISEMENT_TIME_ILLEGAL); // 临时用现有枚举，建议新增
        }
    }
}