 ...
 */
@Component
public class AdminInMemoryDbHybridChatMemory implements ChatMemory {
    // 内存缓存：conversationId -> 消息列表（线程安全）
    private final Map<String, List<Message>> memoryCache = new ConcurrentHashMap<>();

    // 数据库持久化层（注入已配置好Kryo的AdminMySQLBasedChatMemory）
    private final AdminMySQLBasedChatMemory dbChatMemory;

    // 构造器注入（只注入，不处理Kryo）
    public AdminInMemoryDbHybridChatMemory(AdminMySQLBasedChatMemory dbChatMemory) {
