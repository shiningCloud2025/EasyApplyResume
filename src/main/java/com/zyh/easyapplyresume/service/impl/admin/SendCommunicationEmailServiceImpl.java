...
public class SendCommunicationEmailServiceImpl implements SendCommunicationEmailService {
    ...
    @Override
    public void sendHtmlEmailSelfDefition(String fromEmail, String toEmail, String subject, String htmlContent) {
        // 1. 校验输入参数
        if (fromEmail == null || fromEmail.trim().isEmpty()) {
            log.warn("发送HTML邮件失败：发送人邮箱不能为空");
            throw new BusException(AdminCodeEnum.SENDER_EMAIL_NOT_NULL);
        }
        if (toEmail == null || !toEmail.contains("@") || toEmail.split("@").length != 2) {
            log.warn("发送HTML邮件失败：接收人邮箱 [{}] 格式无效", toEmail);
            throw new BusException(AdminCodeEnum.RECIVEDER_EMAIL_NOT_NULL);
        }
        if (subject == null || subject.trim().isEmpty()) {
            log.warn("发送HTML邮件失败：邮件主题不能为空");
            throw new BusException(AdminCodeEnum.SEND_TITLE_NOT_NULL);
        }
        if (htmlContent == null || htmlContent.trim().isEmpty()) {
            log.warn("发送HTML邮件失败：邮件内容不能为空");
            throw new BusException(AdminCodeEnum.SEND_CONTENT_NOT_NULL);
        }
        ...
        try {
            ...
            // 3. 发送邮件
            javaMailSender.send(mimeMessage);
            log.info("向邮箱 [{}] 发送主题为 [{}] 的HTML邮件成功", toEmail, subject);

        } catch (Exception e) {
            log.error("向邮箱 [{}] 发送主题为 [{}] 的HTML邮件失败: ", toEmail, subject, e);
            throw new BusException(AdminCodeEnum.SEND_COMMUNICATION_EMAIL_FAIL);
        }
    }

    @Override
    public void sendHtmlEmailUsallyDefition(String toEmail, String subject, String htmlContent) {
        // 1. 校验输入参数 (注意：这里不需要校验发送者)
        if (toEmail == null || !toEmail.contains("@") || toEmail.split("@").length != 2) {
            log.warn("发送HTML邮件(默认发送者)失败：接收人邮箱 [{}] 格式无效", toEmail);
            throw new BusException(AdminCodeEnum.RECIVEDER_EMAIL_NOT_NULL);
        }
        if (subject == null || subject.trim().isEmpty()) {
            log.warn("发送HTML邮件(默认发送者)失败：邮件主题不能为空");
            throw new BusException(AdminCodeEnum.SEND_TITLE_NOT_NULL);
        }
        if (htmlContent == null || htmlContent.trim().isEmpty()) {
            log.warn("发送HTML邮件(默认发送者)失败：邮件内容不能为空");
            throw new BusException(AdminCodeEnum.SEND_CONTENT_NOT_NULL);
        }
        ...
        try {
            ...
            helper.setTo(toEmail);
            helper.setSubject(subject);
            helper.setText(htmlContent, true); // true 表示内容为HTML

            // 3. 发送邮件
            javaMailSender.send(mimeMessage);
            log.info("使用默认发送者 [{}] 向邮箱 [{}] 发送主题为 [{}] 的HTML邮件成功", defaultFromEmail, toEmail, subject);

        } catch (Exception e) {
            log.error("使用默认发送者 [{}] 向邮箱 [{}] 发送主题为 [{}] 的HTML邮件失败: ", defaultFromEmail, toEmail, subject, e);
            throw new BusException(AdminCodeEnum.SEND_COMMUNICATION_EMAIL_FAIL);
        }
    }
