...
const getList = async () => { loading.value = true; try { const res = await cityMapApi.getCityMapPage(pagination.current, pagination.size, searchForm); tableData.value = res.data.records; pagination.total = res.data.total } catch { ElMessage.error('加载失败') } finally { loading.value = false } }
...
const handleView = async (row: CityMapPageVO) => { const res = await cityMapApi.getCityMapInfo(row.cityMapCid); currentRow.value = res.data; viewVisible.value = true }
onMounted(getList)
</script>
